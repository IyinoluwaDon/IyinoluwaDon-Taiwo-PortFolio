...
function PageIntro({ section }) {
  ...
  return <section className="relative isolate overflow-hidden border-b border-ink/10 px-5 pb-20 pt-32 sm:px-8 lg:px-12 lg:pb-28 lg:pt-40"><div aria-hidden="true" className="slow-orbit absolute -right-20 top-8 -z-10 h-72 w-72 rounded-full border border-dashed border-electric/20 sm:h-96 sm:w-96" /><div aria-hidden="true" className="ambient-pulse absolute -right-8 top-20 -z-10 h-72 w-72 rounded-full bg-electric/10 blur-[100px]" /><div className="mx-auto max-w-7xl"><SectionLabel>{label}</SectionLabel><h1 className="reveal-up max-w-4xl font-display text-5xl font-semibold leading-[0.98] tracking-[-0.065em] text-copy sm:text-7xl">{title}</h1><p className="reveal-up mt-8 max-w-2xl text-base leading-8 text-quiet sm:text-lg" style={{ '--reveal-delay': '120ms' }}>{description}</p></div></section>
}
