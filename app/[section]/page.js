...
const pageMeta = {
  ...
  work: ['03 / Selected work', 'Projects with purpose.', 'A closer look at systems built across applied ML, backend APIs, and full-stack product development.'],
...
function WorkPage() {
  return <div className="mx-auto max-w-7xl px-5 py-24 sm:px-8 lg:px-12 lg:py-32">
    <div className="mb-24 overflow-hidden rounded-3xl border border-white/10 bg-panel p-6 sm:p-10">
      <div className="mb-10 flex flex-wrap items-end justify-between gap-6"><div><p className="text-xs uppercase tracking-[0.2em] text-electric">System map</p><h2 className="mt-4 font-display text-3xl font-semibold tracking-[-0.04em] text-copy sm:text-4xl">From raw data to useful systems.</h2></div><p className="max-w-sm text-sm leading-7 text-quiet">A visual map of the layers I work across: inputs, intelligence, and products people can use.</p></div>
      <div className="relative grid gap-4 md:grid-cols-3">
        <div className="pointer-events-none absolute left-[16%] right-[16%] top-1/2 hidden h-px bg-gradient-to-r from-electric/0 via-electric/60 to-electric/0 md:block" />
        {[['01', 'Inputs', 'Datasets · APIs · messy signals', 'from-blue-500/20'], ['02', 'Intelligence', 'Models · evaluation · iteration', 'from-violet-500/20'], ['03', 'Products', 'Dashboards · APIs · decisions', 'from-cyan-500/20']].map(([number, title, copy, accent]) => <div key={number} className={`relative rounded-2xl border border-white/10 bg-gradient-to-br ${accent} to-transparent p-6`}><span className="text-xs text-electric">{number}</span><h3 className="mt-10 font-display text-2xl font-semibold text-copy">{title}</h3><p className="mt-3 text-sm leading-7 text-quiet">{copy}</p><span className="mt-8 block h-1 w-12 rounded-full bg-electric/70" /></div>)}
      </div>
      <div className="mt-6 flex flex-wrap gap-2 text-xs text-quiet">{['Python', 'SQL', 'scikit-learn', 'XGBoost', 'AWS', 'Flask', 'Streamlit'].map((item) => <span key={item} className="rounded-full border border-white/10 px-3 py-1">{item}</span>)}</div>
    </div>
    <div className="space-y-5">{projects.map((project) => <article key={project.number} className="rounded-2xl border border-white/10 bg-panel p-6 sm:p-10"><div className="grid gap-8 lg:grid-cols-[80px_1fr_auto]"><span className="text-sm text-electric">{project.number}</span><div><p className="text-xs uppercase tracking-[0.16em] text-quiet">{project.type}</p><h2 className="mt-4 font-display text-3xl font-semibold text-copy">{project.title}</h2><p className="mt-5 max-w-2xl text-sm leading-8 text-quiet">{project.description}</p><div className="mt-6 flex flex-wrap gap-2">{project.stack.map((item) => <span key={item} className="rounded-full border border-white/10 px-3 py-1 text-xs text-quiet">{item}</span>)}</div></div><div className="flex gap-5"><a href={project.github} target="_blank" rel="noreferrer" className="text-sm text-copy hover:text-electric">View code <Arrow /></a>{project.demo && <a href={project.demo} target="_blank" rel="noreferrer" className="text-sm text-copy hover:text-electric">Live demo <Arrow /></a>}</div></div></article>)}</div>
  </div>
}
