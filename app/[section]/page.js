...
function ExperiencePage() {
  return <div className="mx-auto max-w-7xl px-5 py-24 sm:px-8 lg:px-12 lg:py-32"><div className="divide-y divide-ink/10">{experience.map(([period, role, org, description]) => <article key={role} className="grid gap-5 py-10 md:grid-cols-[180px_1fr] md:gap-12"><p className="text-xs text-quiet">{period}</p><div><h2 className="font-display text-2xl font-semibold text-copy">{role}</h2><p className="mt-2 text-sm text-electric">{org}</p><p className="mt-5 max-w-2xl text-sm leading-8 text-quiet">{description}</p></div></article>)}</div></div>
}

function CredentialsPage() {
  if (certificates.length === 0) {
    return <div className="mx-auto max-w-7xl px-5 py-24 sm:px-8 lg:px-12 lg:py-32"><div className="max-w-2xl border-l-2 border-electric pl-6"><p className="text-lg leading-9 text-copy">Certificate records are being added. Check back soon for verified learning milestones and credential links.</p></div></div>
  }
  return <div className="mx-auto max-w-7xl px-5 py-24 sm:px-8 lg:px-12 lg:py-32"><div className="grid gap-5 md:grid-cols-2">{certificates.map((certificate) => <a key={certificate.name} href={certificate.url} target="_blank" rel="noreferrer" className="rounded-xl border border-ink/10 bg-panel p-7 transition-colors hover:border-electric"><p className="text-xs uppercase tracking-[0.16em] text-electric">{certificate.issuer}</p><h2 className="mt-5 font-display text-2xl font-semibold text-copy">{certificate.name}</h2><p className="mt-7 text-sm font-medium text-quiet">View credential <Arrow /></p></a>)}</div></div>
}

function ContactPage() {
  return <div className="mx-auto max-w-7xl px-5 py-24 sm:px-8 lg:px-12 lg:py-36"><div className="max-w-3xl rounded-2xl border border-ink/10 bg-panel p-8 sm:p-12"><p className="max-w-xl text-base leading-8 text-quiet">Open to ML internships, research collaborations, and freelance ML engineering work. If you are building with AI - models, pipelines, or APIs - reach out.</p><div className="mt-8 flex flex-wrap gap-3"><a href={`mailto:${links.email}`} className="inline-flex rounded-lg bg-ink px-5 py-3 text-sm font-semibold text-white">{links.email}</a><a href={links.resume} target="_blank" rel="noreferrer" className="inline-flex rounded-lg border border-ink/20 px-5 py-3 text-sm text-copy hover:border-electric hover:text-electric">View résumé <Arrow /></a></div><div className="mt-8 flex gap-6 text-sm text-quiet"><a href={links.github} target="_blank" rel="noreferrer" className="hover:text-electric">GitHub <Arrow /></a><a href={links.linkedin} target="_blank" rel="noreferrer" className="hover:text-electric">LinkedIn <Arrow /></a></div></div></div>
}
